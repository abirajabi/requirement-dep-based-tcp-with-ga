...
def evaluation(pop):
    ...
    for i, chromosome in enumerate(pop):
        ...
        for j in range(2):
            if j == 0:      # objective 1
                fitness_values[i, j] = 1 - \
                    math.exp(-sum((chromosome - 1/math.sqrt(n))**2))
...
def crowding_calculation(fitness_values):
    ...
    for i in range(fitness_value_number):
        ...
        sorting_normalize_fitness_values = np.sort(
            normalize_fitness_values[:, i])
        sorting_normalized_values_index = np.argsort(
            normalize_fitness_values[:, i])
        # crowding distance calculation
        crowding_results[1:pop_size - 1] = (sorting_normalize_fitness_values[2:pop_size] -
                                            sorting_normalize_fitness_values[0:pop_size - 2])
        # re_sorting to the orginal order
...
def remove_using_crowding(fitness_values, number_solutions_needed):
    ...
    for i in range(number_solutions_needed):
        ...
        if crowding_distance[solution_1] >= crowding_distance[solution_2]:
            ...
            # remove the fitness of the selected solution
            fitness_values = np.delete(fitness_values, (solution_1), axis=0)
            # remove the related crowding distance
    ...
    for i in range(pop_size):
        for j in range(pop_size):
            if all(fitness_values[j] <= fitness_values[i]) and any(fitness_values[j] < fitness_values[i]):
                # i is not in pareto front because j dominates i
                pareto_front[i] = 0
...
def selection(pop, fitness_values, pop_size):
    ...
    while len(pareto_front_index) < pop_size:
        ...
        pareto_front_index = np.hstack(
            (pareto_front_index, new_pareto_front))  # add to pareto
